...
!     e-mail: amiller @ bigpond.net.au
   ...
   IMPLICIT NONE
   real(dp), PRIVATE      :: zero = 0.0, half = 0.5, one = 1.0, &
                             two = 2.0, vsmall = TINY(1.0), vlarge = HUGE(1.0)
   PRIVATE            :: integral
   INTEGER, PARAMETER :: dp_fmt = SELECTED_REAL_KIND(12, 60)
...
!     Local variables
      real(dp), PARAMETER  :: aln4 = 1.3862944d0
      real(dp)             :: a, b, g, r, s, x, y, z
      real(dp), SAVE       :: d, f, h, t, c
      LOGICAL, SAVE    :: swap
...
!     Local variables
      ...
      real(dp), PARAMETER :: three = 3d0, four = 4d0, quart = 0.25d0, &
                             five = 5d0, sixteen = 16d0
      INTEGER         :: mm = 0
...
!        = 0 otherwise
      ...
      INTEGER, INTENT(IN)   :: n
      real(dp), INTENT(IN)      :: h(:), d(:)   ! d(n*(n+1)/2)
      real(dp), INTENT(IN OUT)  :: f(:)         ! f(n*(n+1)/2)
...
!     Local variables
      real(dp)            :: ym, xm, r, w, r1, r2, x
      real(dp), SAVE      :: a, c, d, e
...
!     ..
!     .. Data statements ..
      real(dp), PARAMETER :: a0 = -.5d0, a1 = .3333333d0, a2 = -.2500068d0, a3 = .2000118d0, &
                             a4 = -.1661269d0, a5 = .1421878d0, a6 = -.1384794d0, &
                             a7 = .1250060d0

      real(dp), PARAMETER :: fact(10) = (/1d0, 1d0, 2d0, 6d0, 24d0, 120d0, 720d0, 5040d0, &
                                          40320d0, 362880d0/)
...
!             C=.1069/MU GUARANTEES MAJORIZATION BY THE 'HAT'-FUNCTION.
         ...
         IF (.NOT. full_init) THEN
            ...
            c3 = .1428571d0*b1*b2
            c2 = b2 - 15d0*c3
            c1 = b1 - 6d0*b2 + 45d0*c3
            c0 = 1d0 - b1 + 3d0*b2 - 15d0*c3
            c = .1069d0/mu
            full_init = .true.
         END IF
...
80       del = .8333333d-1/fk
         del = del - 4.8d0*del*del*del
         v = difmuk/fk
         IF (ABS(v) > 0.25d0) THEN
            px = fk*LOG(one + v) - difmuk - del
         ELSE
            px = fk*v*v*(((((((a7*v + a6)*v + a5)*v + a4)*v + a3)*v + a2)*v + a1)*v + a0) - del
         END IF
         py = .3989423d0/SQRT(fk)
110      x = (half - difmuk)/s
         xx = x*x
         fx = -half*xx
...
!     Calculate a binomial probability
      ...
      fn_val = EXP(lngamma(real(n + 1, kind=dp)) - lngamma(real(r + 1, kind=dp)) &
                   - lngamma(real(n - r + 1, kind=dp)) + r*LOG(p) + (n - r)*LOG(one - p))
      RETURN
...
!  N.B. The real(dp) Stirling's formula is used here, not the simpler, but less
...
!     ..
!     .. Local Scalars ..
      real(dp)            :: alv, amaxp, f, f1, f2, u, v, w, w2, x, x1, x2, ynorm, z, z2
      INTEGER         :: i, ix, ix1, k, mp
      INTEGER, SAVE   :: m
      real(dp), SAVE      :: p, q, xnp, ffm, fm, xnpq, p1, xm, xl, xr, c, al, xll, &
                             xlr, p2, p3, p4, qn, r, g
...
!*****SETUP, PERFORM ONLY WHEN PARAMETERS CHANGE
      ...
      IF (xnp > 30.) THEN
         IF (first) THEN
            ...
            fm = m
            xnpq = xnp*q
            p1 = INT(2.195d0*SQRT(xnpq) - 4.6d0*q) + half
            xm = fm + half
            xl = xm - p1
            xr = xm + p1
            c = 0.134d0 + 20.5d0/(15.3d0 + fm)
            al = (ffm - xl)/(ffm - xl*p)
            xll = al*(one + half*al)
            al = (xr - ffm)/(xr*q)
            xlr = al*(one + half*al)
...
!     SQUEEZING USING UPPER AND LOWER BOUNDS ON LOG(F(X))
         ...
         amaxp = (k/xnpq)*((k*(k/3d0 + .625d0) + .1666666666666d0)/xnpq + half)
         ynorm = -k*k/(2d0*xnpq)
         alv = LOG(v)
...
!     THE FINAL ACCEPTANCE/REJECTION TEST
         ...
         IF (alv - (xm*LOG(f1/x1) + (n - m + half)*LOG(z/w) + (ix - m)*LOG(w*p/(x1*q)) + &
                    (13860d0 - (462d0 - (132.-(99d0 - 140d0/f2)/f2)/f2)/f2)/f1/166320d0 + &
                    (13860d0 - (462d0 - (132.-(99d0 - 140d0/z2)/z2)/z2)/z2)/z/166320d0 + &
                    (13860d0 - (462d0 - (132.-(99d0 - 140d0/x2)/x2)/x2)/x2)/x1/166320d0 + &
                    (13860d0 - (462d0 - (132.-(99d0 - 140d0/w2)/w2)/w2)/w2)/w/166320d0) > zero) THEN
            GO TO 20
         ELSE
            GO TO 110
         END IF
...
! THE REPRODUCTIVE PROPERTY IS USED.
      ...
      real(dp), INTENT(IN)   :: sk, p
      INTEGER            :: ival

!     Local variables
! THE PARAMETER ULN = -LOG(MACHINE'S SMALLEST real(dp) NUMBER).

      real(dp), PARAMETER    :: h = 0.7d0
      real(dp)               :: q, x, st, uln, v, r, s, y, g
      INTEGER            :: k, i, n
...
!     Local variables
      ...
      real(dp), SAVE       :: p(20), theta(0:20)
      real(dp)             :: sump, r, th, lambda, rlast
      real(dp)         :: dk
...
!     Local variables
      ...
      real(dp)   :: xmid, range, x1, x2, &
                    x(3) = (/0.238619186083197_dp_fmt, 0.661209386466265_dp_fmt, 0.932469514203152_dp_fmt/), &
                    w(3) = (/0.467913934572691_dp_fmt, 0.360761573048139_dp_fmt, 0.171324492379170_dp_fmt/)
      INTEGER    :: i
